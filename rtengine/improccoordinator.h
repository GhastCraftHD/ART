...
protected:
    ...
    Imagefloat
        *drcomp_11_dcrop_cache; // global cache for dynamicRangeCompression used
                                // in 1:1 detail windows (except when denoise is
                                // active)
    Image8 *previmg; // displayed image in monitor color space, showing the
                     // output profile as well (soft-proofing enabled, which
                     // then correspond to workimg) or not
...
public:
    ...
    void getParams(procparams::ProcParams *dst) override { *dst = params; }
    ...
    void setMonitorProfile(const Glib::ustring &profile,
                           RenderingIntent intent) override;
