...
#pragma omp parallel for
        ...
        for (int row = 0; row < H; row++) {
            ...
            for (int col = radx + 1; col < W - radx; col++) {
                temp[row * W + col] =
                    temp[row * W + col - 1] +
                    ((float)(src[row][col + radx] - src[row][col - radx - 1])) /
                        len;
            }

            for (int col = W - radx; col < W; col++) {
                temp[row * W + col] =
                    (temp[row * W + col - 1] * len - src[row][col - radx - 1]) /
                    (len - 1);
                len--;
            }
        }
...
#pragma omp parallel for
        ...
        for (int col = 0; col < W; col++) {
            ...
            for (int row = 1; row <= rady; row++) {
                dst[row][col] =
                    (dst[(row - 1)][col] * len + temp[(row + rady) * W + col]) /
                    (len + 1);
                len++;
            }
            ...
            for (int row = rady + 1; row < H - rady; row++) {
                dst[row][col] =
                    dst[(row - 1)][col] + (temp[(row + rady) * W + col] -
                                           temp[(row - rady - 1) * W + col]) /
                                              len;
            }

            for (int row = H - rady; row < H; row++) {
                dst[row][col] = (dst[(row - 1)][col] * len -
                                 temp[(row - rady - 1) * W + col]) /
                                (len - 1);
                len--;
            }
        ...
        for (int row = 0; row < H; row++) {
            ...
            for (int col = 1; col <= radx; col++) {
                temp[row * W + col] = tempval =
                    (tempval * len + (float)src[row][col + radx]) / (len + 1);
                len++;
            }
            ...
            for (int col = W - radx; col < W; col++) {
                temp[row * W + col] = tempval =
                    (tempval * len - src[row][col - radx - 1]) / (len - 1);
                len--;
            }
        }
    }
...
#pragma omp for
        ...
        for (int col = 0; col < W - 7; col += 8) {
            ...
            for (int row = 1; row <= rady; row++) {
                lenp1v = lenv + onev;
                tempv = (tempv * lenv + LVFU(temp[(row + rady) * W + col])) /
                        lenp1v;
                temp1v =
                    (temp1v * lenv + LVFU(temp[(row + rady) * W + col + 4])) /
                    lenp1v;
            ...
            for (int row = rady + 1; row < H - rady; row++) {
                tempv = tempv + (LVFU(temp[(row + rady) * W + col]) -
                                 LVFU(temp[(row - rady - 1) * W + col])) *
                                    rlenv;
                temp1v = temp1v + (LVFU(temp[(row + rady) * W + col + 4]) -
                                   LVFU(temp[(row - rady - 1) * W + col + 4])) *
                                      rlenv;
                STVFU(dst[row][col], tempv);
                STVFU(dst[row][col + 4], temp1v);
            }
            ...
            for (int row = H - rady; row < H; row++) {
                lenm1v = lenv - onev;
                tempv =
                    (tempv * lenv - LVFU(temp[(row - rady - 1) * W + col])) /
                    lenm1v;
                temp1v = (temp1v * lenv -
                          LVFU(temp[(row - rady - 1) * W + col + 4])) /
                         lenm1v;
...
#pragma omp for
        ...
        for (int col = 0; col < W - numCols + 1; col += 8) {
            ...
            for (int row = 1; row <= rady; row++) {
                for (int k = 0; k < numCols; k++) {
                    dst[row][col + k] = (dst[(row - 1)][col + k] * len +
                                         temp[(row + rady) * W + col + k]) /
                                        (len + 1);
                }
            ...
            for (int row = rady + 1; row < H - rady; row++) {
                for (int k = 0; k < numCols; k++) {
                    dst[row][col + k] = dst[(row - 1)][col + k] +
                                        (temp[(row + rady) * W + col + k] -
                                         temp[(row - rady - 1) * W + col + k]) /
                                            len;
                }
            }
...
inline void boxblur(float **src, float **dst, int radius, int W, int H,
                    bool multiThread)
{
...
#pragma omp parallel for if (multiThread)
    ...
    constexpr int numCols =
        8; // process numCols columns at once for better usage of L1 cpu cache
#ifdef _OPENMP
...
#pragma omp for
        ...
        for (int row = 0; row < H; row++) {
            ...
            for (int col = radius + 1; col < W - radius; col++) {
                ...
                dst[row][col] = tempval =
                    tempval + (src[row][col + radius] - oldVal) / len;
                ++pos;
            ...
            for (int col = W - radius; col < W; col++) {
                dst[row][col] = tempval =
                    (tempval * len - lineBuffer[pos]) / (len - 1);
                --len;
                ++pos;
                pos = pos <= radius ? pos : 0;
            }
...
#pragma omp for nowait
        ...
        for (int col = 0; col < W - 7; col += 8) {
            ...
            for (int row = 1; row <= radius; row++) {
                ...
                temp1v =
                    (temp1v * lenv + LVFU(dst[row + radius][col + 4])) / lenp1v;
                STVFU(dst[row][col], tempv);
                STVFU(dst[row][col + 4], temp1v);
                lenv = lenp1v;
            }
            ...
            for (int row = radius + 1; row < H - radius; row++) {
                ...
                tempv =
                    tempv + (LVFU(dst[row + radius][col]) - oldVal0) * rlenv;
                temp1v = temp1v +
                         (LVFU(dst[row + radius][col + 4]) - oldVal1) * rlenv;
                STVFU(dst[row][col], tempv);
                STVFU(dst[row][col + 4], temp1v);
                ++pos;
                pos = pos <= radius ? pos : 0;
        ...
        for (int col = 0; col < W - numCols + 1; col += 8) {
            ...
            for (int row = radius + 1; row < H - radius; row++) {
                for (int k = 0; k < numCols; k++) {
                    ...
                    dst[row][col + k] =
                        dst[row - 1][col + k] +
                        (dst[row + radius][col + k] - oldVal) / len;
                }
                ++pos;
                pos = pos <= radius ? pos : 0;
            }
            ...
            for (int row = H - radius; row < H; row++) {
                for (int k = 0; k < numCols; k++) {
                    dst[row][col + k] =
                        (dst[row - 1][col + k] * len - rowBuffer[pos][k]) /
                        (len - 1);
                }
                len--;
                ++pos;
            ...
            if (remaining > 0) {
                ...
                for (int row = radius + 1; row < H - radius; ++row) {
                    for (int k = 0; k < remaining; ++k) {
                        ...
                        dst[row][col + k] =
                            dst[row - 1][col + k] +
                            (dst[row + radius][col + k] - oldVal) * rlen;
                    }
                    ++pos;
                    pos = pos <= radius ? pos : 0;
                }
...
template <class T, class A>
void boxblur(T *src, A *dst, A *buffer, int radx, int rady, int W, int H)
{
    ...
    if (radx == 0) {
        ...
        for (int row = H - 1; row >= 0; row--) {
            ...
            for (int col = 1; col <= radx; col++) {
                tempval =
                    (tempval * len + src[row * W + col + radx]) / (len + 1);
                temp[row * W + col] = tempval;
                len++;
            }
            ...
            for (int col = radx + 1; col < W - radx; col++) {
                tempval = tempval + ((float)(src[row * W + col + radx] -
                                             src[row * W + col - radx - 1])) *
                                        reclen;
                temp[row * W + col] = tempval;
            }
            ...
            for (int col = W - radx; col < W; col++) {
                tempval =
                    (tempval * len - src[row * W + col - radx - 1]) / (len - 1);
                temp[row * W + col] = tempval;
                len--;
            }
        }
    ...
    if (rady == 0) {
        ...
        for (; col < W; col++) {
            ...
            for (int row = 1; row <= rady; row++) {
                dst[row * W + col] = (dst[(row - 1) * W + col] * len +
                                      temp[(row + rady) * W + col]) /
                                     (len + 1);
                len++;
            }
...
template <class T, class A>
void boxabsblur(T *src, A *dst, int radx, int rady, int W, int H, float *temp)
{
    ...
    if (radx == 0) {
        ...
        for (int row = 0; row < H; row++) {
            ...
            for (int col = 1; col <= radx; col++) {
                tempval = (tempval * len + fabsf(src[row * W + col + radx])) /
                          (len + 1);
                temp[row * W + col] = tempval;
                len++;
            }
            ...
            for (int col = radx + 1; col < W - radx; col++) {
                tempval =
                    tempval + ((float)(fabsf(src[row * W + col + radx]) -
                                       fabsf(src[row * W + col - radx - 1]))) *
                                  rlen;
                temp[row * W + col] = tempval;
            }
            ...
            for (int col = W - radx; col < W; col++) {
                tempval =
                    (tempval * len - fabsf(src[row * W + col - radx - 1])) /
                    (len - 1);
                temp[row * W + col] = tempval;
                len--;
            }
    ...
    if (rady == 0) {
        ...
        for (int col = 0; col < W - 3; col += 4) {
            ...
            for (int row = 1; row <= rady; row++) {
                lenp1v = lenv + onev;
                tempv =
                    (tempv * lenv + LVF(temp[(row + rady) * W + col])) / lenp1v;
            ...
            for (int row = rady + 1; row < H - rady; row++) {
                tempv = tempv + (LVF(temp[(row + rady) * W + col]) -
                                 LVF(temp[(row - rady - 1) * W + col])) *
                                    rlenv;
                STVF(dst[row * W + col], tempv);
            }
            ...
            for (int row = H - rady; row < H; row++) {
                lenm1v = lenv - onev;
                tempv = (tempv * lenv - LVF(temp[(row - rady - 1) * W + col])) /
                        lenm1v;
                STVF(dst[row * W + col], tempv);
                lenv = lenm1v;
            }
