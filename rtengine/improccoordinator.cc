...
ImProcCoordinator::ImProcCoordinator()
    : orig_prev(nullptr), oprevi(nullptr), spotprev(nullptr),

      drcomp_11_dcrop_cache(nullptr), previmg(nullptr), workimg(nullptr),
      imgsrc(nullptr), lastAwbEqual(0.), ipf(&params, true),
      monitorIntent(RI_RELATIVE), softProof(false), gamutCheck(GAMUT_CHECK_OFF),
      sharpMask(false), scale(10), highDetailPreprocessComputed(false),
      highDetailRawComputed(false), allocated(false),

      vhist16(65536), histRed(256), histRedRaw(256), histGreen(256),
      histGreenRaw(256), histBlue(256), histBlueRaw(256), histLuma(256),
      histToneCurve(256), histLCurve(256), histCCurve(256),
      // histLLCurve(256),
      histLCAM(256), histCCAM(256), histChroma(256), histLRETI(256),

      hist_lrgb_dirty(false), hist_raw_dirty(false), vectorscopeScale(0),
      vectorscope_hc_dirty(false), vectorscope_hs_dirty(false),
      vectorscope_hc(VECTORSCOPE_SIZE, VECTORSCOPE_SIZE),
      vectorscope_hs(VECTORSCOPE_SIZE, VECTORSCOPE_SIZE), waveformScale(0),
      waveform_dirty(false), waveformRed(0, 0), waveformGreen(0, 0),
      waveformBlue(0, 0), waveformLuma(0, 0),

      fw(0), fh(0), tr(0), fullw(1), fullh(1), pW(-1), pH(-1),
      plistener(nullptr), imageListener(nullptr), aeListener(nullptr),
      awbListener(nullptr), flatFieldAutoClipListener(nullptr),
      bayerAutoContrastListener(nullptr), xtransAutoContrastListener(nullptr),
      frameCountListener(nullptr), imageTypeListener(nullptr),
      filmNegListener(nullptr), adnListener(nullptr), hListener(nullptr),
      autoLogListener(nullptr), autoRadiusListener(nullptr), resultValid(false),
      tweakOperator(nullptr), lastOutputProfile("BADFOOD"),
      lastOutputIntent(RI__COUNT), lastOutputBPC(false),
      // thread(nullptr),
...
void ImProcCoordinator::restoreParams() { params = paramsBackup; }

// todo: bitmask containing desired actions, taken from changesSinceLast
// cropCall: calling crop, used to prevent self-updates  ...doesn't seem to be
// used
void ImProcCoordinator::updatePreviewImage(int todo, bool panningRelatedChange)
{
    ...
    if (todo == CROP && ipf.needsPCVignetting()) {
        todo |= M_LUMINANCE; // TRANSFORM;    // Change about Crop does affect
                             // TRANSFORM
    }

    bool highDetailNeeded = false;
    // Check if any detail crops need high detail. If not, take a fast path
    // short cut
    if (!highDetailNeeded) {
    ...
    if (((todo & ALL) == ALL) || (todo & M_MONITOR) || panningRelatedChange ||
        ...
        if (!highDetailNeeded) {
            ...
            if (rp.bayersensor.method != RAWParams::BayerSensor::Method::NONE &&
                rp.bayersensor.method != RAWParams::BayerSensor::Method::MONO) {
                rp.bayersensor.method = RAWParams::BayerSensor::Method::FAST;
            }

            // bayerrp.all_enhance = false;

            if (rp.xtranssensor.method !=
                    RAWParams::XTransSensor::Method::NONE &&
                rp.xtranssensor.method !=
        ...
        progress(
            "Applying white balance, color correction & sRGB conversion...",
            100 * readyphase / numofphases);

        if (frameCountListener) {
            frameCountListener->FrameCountChanged(
                imgsrc->getFrameCount(), params.raw.bayersensor.imageNum);
        }
        ...
        // raw auto CA is bypassed if no high detail is needed, so we have to
        // compute it when high detail is needed
        if ((todo & M_PREPROC) ||
            (!highDetailPreprocessComputed && highDetailNeeded)) {
            imgsrc->preprocess(rp, params.lensProf, params.coarse, true,
                               preproc_wb);
            if (flatFieldAutoClipListener && rp.ff_AutoClipControl) {
                flatFieldAutoClipListener->flatFieldAutoClipValueChanged(
        ...
        if (imageTypeListener) {
            imageTypeListener->imageTypeChanged(
                imgsrc->isRAW(), imgsrc->getSensorType() == ST_BAYER,
                imgsrc->getSensorType() == ST_FUJI_XTRANS, imgsrc->isMono());
        }

        if ((todo & M_RAW) || (!highDetailRawComputed && highDetailNeeded)) {
            if (settings->verbose) {
                if (imgsrc->getSensorType() == ST_BAYER) {
                    std::cout << "Demosaic Bayer image n."
                              << rp.bayersensor.imageNum + 1
                              << " using method: "
                              << RAWParams::BayerSensor::getMethodString(
                                     rp.bayersensor.method)
                              << std::endl;
                } else if (imgsrc->getSensorType() == ST_FUJI_XTRANS) {
                    std::cout << "Demosaic X-Trans image with using method: "
            ...
            if (imgsrc->getSensorType() == ST_BAYER) {
                if (params.raw.bayersensor.method !=
                    ...
                    imgsrc->setBorder(
                        std::max(params.raw.bayersensor.border, 2));
                }
            } else if (imgsrc->getSensorType() == ST_FUJI_XTRANS) {
            ...
            imgsrc->demosaic(rp, autoContrast,
                             contrastThreshold); // enabled demosaic

            if (imgsrc->getSensorType() == ST_BAYER &&
                bayerAutoContrastListener && autoContrast) {
                bayerAutoContrastListener->autoContrastChanged(
                    autoContrast ? contrastThreshold : -1.0);
            }
            if (imgsrc->getSensorType() == ST_FUJI_XTRANS &&
                xtransAutoContrastListener && autoContrast) {
                xtransAutoContrastListener->autoContrastChanged(
                    autoContrast ? contrastThreshold : -1.0);
            }

            // if a demosaic happened we should also call getimage later, so we
            // need to set the M_INIT flag
            todo |= M_INIT;
        ...
        if (todo & (M_INIT | M_LINDENOISE | M_HDR)) {
            ...
            if (params.wb.method == WBParams::AUTO && awbListener &&
                params.wb.enabled) {
                awbListener->WBChanged(currWB);
            }

            // setScale(scale);
            imgsrc->getImage(currWB, tr, orig_prev, pp, params.exposure,
                             params.raw);
            // if (todo & M_INIT) {
            ...
            // Perform negative inversion. If needed, upgrade filmNegative
            // params for backwards compatibility with old profiles
            if (params.filmNegative.enabled) {
                if (ipf.filmNegativeProcess(orig_prev, orig_prev,
                                            params.filmNegative, params.raw,
                                            imgsrc, currWB) &&
                    filmNegListener) {
                    filmNegListener->filmRefValuesChanged(
                        params.filmNegative.refInput,
                        params.filmNegative.refOutput);
                }
            }
            if (!converted) {
                imgsrc->convertColorSpace(orig_prev, params.icm, currWB);
        ...
        if (todo & M_SPOT) {
            if (params.spot.enabled && !params.spot.entries.empty()) {
                ...
                PreviewProps pp(0, 0, fw, fh, scale);
                ipf.removeSpots(spotprev, imgsrc, params.spot.entries, pp,
                                currWB, &params.icm, tr, nullptr);
            } else {
                if (spotprev) {
                    delete spotprev;
                    spotprev = nullptr;
        ...
        if ((todo & M_HDR) &&
            (params.fattal.enabled || params.dehaze.enabled)) {
            if (drcomp_11_dcrop_cache) {
                delete drcomp_11_dcrop_cache;
                drcomp_11_dcrop_cache = nullptr;
            }
        ...
        readyphase++;
        progress("Preparing shadow/highlight map...",
                 100 * readyphase / numofphases);
        ...
        if ((todo & M_INIT) && autoRadiusListener) {
            if (!imgsrc->getDeconvAutoRadius(nullptr)) {
                ...
                autoRadiusListener->autoDeconvRadiusChanged(
                    params.sharpening.deconvradius);
            }
        }
        ...
        if (todo & M_AUTOEXP) {
            if (params.toneCurve.histmatching) {
                ...
                if (aeListener) {
                    aeListener->autoMatchedToneCurveChanged(
        ...
        progress("Exposure curve & CIELAB conversion...",
                 100 * readyphase / numofphases);

        if ((todo & M_RGBCURVE) || (todo & M_CROP)) {
            // if it's just crop we just need the histogram, no image updates
            if (todo & M_RGBCURVE) {
                // initialize rrm bbm ggm different from zero to avoid black
        ...
        if ((todo & M_MONITOR) ||
            ...
            lastOutputIntent = params.icm.outputIntent;
            lastOutputBPC = params.icm.outputBPC;
            ipf.updateColorProfiles(monitorProfile, monitorIntent, softProof,
                                    gamutCheck);
        }
    }
    ...
    if (panningRelatedChange || (todo & M_MONITOR)) {
        ...
        if ((todo != CROP && todo != MINUPDATE) || (todo & M_MONITOR)) {
            ...
            try {
                ...
                // WCS->Monitor color space (soft-proofing disabled) or
                // WCS->Printer profile->Monitor color space (soft-proofing
                // enabled)
                ipf.rgb2monitor(bufs_[2], previmg);

                // Computing the internal image for analysis, i.e. conversion
        ...
        if (imageListener)
        // TODO: The WB tool should be advertised too in order to get the
        // AutoWB's temp and green values
        {
        ...
        hist_lrgb_dirty = vectorscope_hc_dirty = vectorscope_hs_dirty =
            waveform_dirty = true;
        if (hListener) {
...
/** @brief Handles image buffer (re)allocation and trigger sizeChanged of
 * SizeListener[s] If the scale change, this method will free all buffers and
 * reallocate ones of the new size. It will then tell to the SizeListener that
 * size has changed (sizeChanged)
 *
...
void ImProcCoordinator::setScale(int prevscale)
{
    ...
    } while (nH < 400 && prevscale > 1 &&
             (nW * nH < 1000000)); // sctually hardcoded values, perhaps a
                                   // better choice is possible
...
void ImProcCoordinator::notifyHistogramChanged()
{
    if (hListener) {
        hListener->histogramChanged(
            histRed, histGreen, histBlue, histLuma, histToneCurve, histLCurve,
            histCCurve, histLCAM, histCCAM, histRedRaw, histGreenRaw,
            histBlueRaw, histChroma, histLRETI, vectorscopeScale,
            vectorscope_hc, vectorscope_hs, waveformScale, waveformRed,
            waveformGreen, waveformBlue, waveformLuma);
    }
}
...
#endif
        {
            ...
            for (int i = y1; i < y2; i++)
                for (int j = x1; j < x2; j++) {
                    ...
                    histChroma[(int)(sqrtf(SQR(a) + SQR(b)) /
                                     188.f)]++; // 188 = 48000/256
...
void rgb2lab(const Image8 &src, int x, int y, int w, int h, float L[],
             float a[], float b[], const procparams::ColorManagementParams &icm)
{ // Adapted from ImProcFunctions::lab2rgb
    ...
    if (oprof) {
        ...
        cmsHTRANSFORM hTransform = cmsCreateTransform(
            oprof, TYPE_RGB_8, LabIProf, TYPE_Lab_FLT, icm.outputIntent, flags);
...
#pragma omp parallel for schedule(dynamic, 16) // if (multiThread)
        ...
        for (int i = y; i < y2; i++) {
            ...
            for (int j = x; j < x2; j++) {
                ...
                Color::rgbxyz(igamma[rgb_factor * src.r(i, j)],
                              igamma[rgb_factor * src.g(i, j)],
                              igamma[rgb_factor * src.b(i, j)], X, Y, Z, wp);
...
bool ImProcCoordinator::updateVectorscopeHC()
{
    ...
    rgb2lab(*workimg, x1, y1, x2 - x1, y2 - y1, L.get(), a.get(), b.get(),
            params.icm);
...
#endif
        for (int i = y1; i < y2; ++i) {
            for (int j = x1, ofs_lab = (i - y1) * (x2 - x1); j < x2;
                 ++j, ++ofs_lab) {
...
bool ImProcCoordinator::getAutoWB(ColorTemp &out, double equal)
{
    ...
    if (imgsrc) {
        if (lastAwbEqual != equal) {
            // Issue 2500            MyMutex::MyLock lock(minit);  // Also used
            // in crop window
            double rm, gm, bm;
            imgsrc->getAutoWBMultipliers(rm, gm, bm);
...
void ImProcCoordinator::getAutoCrop(double ratio, int &x, int &y, int &w,
                                    int &h)
{
    ...
    if (params.lensProf.useLcp() && imgsrc->getMetaData()->getFocalLen() > 0) {
        ...
        if (pLCPProf)
            pLCPMap =
                new LCPMapper(pLCPProf, imgsrc->getMetaData()->getFocalLen(),
                              imgsrc->getMetaData()->getFocalLen35mm(),
                              imgsrc->getMetaData()->getFocusDist(), 0, false,
                              params.lensProf.useDist, fullw, fullh,
                              params.coarse, imgsrc->getRotateDegree());
    }
    ...
    if (params.perspective.enabled && !params.commonTrans.autofill) {
        int xx, yy, ww, hh;
        PerspectiveCorrection::autocrop(w, h, ratio > 0, params.perspective,
                                        imgsrc->getMetaData(), xx, yy, ww, hh);
        x += xx;
        y += yy;
        w = ww;
        h = hh;
...
void ImProcCoordinator::setMonitorProfile(const Glib::ustring &profile,
...
void ImProcCoordinator::getMonitorProfile(Glib::ustring &profile,
    ...
    if (flags & (LINKEDMASK & ~LINKEDMASK_FIRST)) {
        for (auto p : pp.get_maskable()) {
            for (auto &m : p->get_masks()) {
                if (m.linkedMask.enabled && !m.linkedMask.toolname.empty() &&
                    !m.linkedMask.name.empty()) {
                    flags |= LINKEDMASK;
                    goto out;
                }
            }
...
bool ImProcCoordinator::getHighQualComputed()
{
    ...
    if (!highQualityComputed) {
        if (options.prevdemo == PD_Sidecar &&
            ...
            for (size_t i = 0; i < crops.size() - 1;
                 ++i) { // -1, because last entry is the freshly created detail
                        // window
                if (crops[i]->get_skip() ==
                    1) { // there is at least one crop with skip == 1 => we
                         // already have high quality preview
                    setHighQualComputed();
                    break;
                }
            }
...
#define CHECK_MASK_(p)                                                         \
    if (p.enabled && p.showMask >= 0 && size_t(p.showMask) < p.masks.size() && \
        p.masks[p.showMask].enabled) {                                         \
        return true;                                                           \
    }
