...
// This sRGB gamma is taken from DNG reference code, with the added linear
// extension past 1.0, as we run clipless here
...
DCPProfile::Matrix invert3x3(const DCPProfile::Matrix &a)
{
    ...
    if (std::fabs(det) < 1.0e-10 && settings->verbose) {
        std::cerr << "DCP matrix cannot be inverted! Expect weird output."
                  << std::endl;
        return a;
    }
...
DCPProfile::Matrix multiply3x3(const DCPProfile::Matrix &a,
                               const DCPProfile::Matrix &b)
{
    DCPProfile::Matrix res;
...
DCPProfile::Triple multiply3x3_v3(const DCPProfile::Matrix &a,
                                  const DCPProfile::Triple &b)
{
...
DCPProfile::Matrix mix3x3(const DCPProfile::Matrix &a, double mul_a,
                          const DCPProfile::Matrix &b, double mul_b)
{
    DCPProfile::Matrix res;
...
DCPProfile::Matrix mapWhiteMatrix(const DCPProfile::Triple &white1,
                                  const DCPProfile::Triple &white2)
{
    // Code adapted from dng_color_spec::MapWhiteMatrix

    // Use the linearized Bradford adaptation matrix
    const DCPProfile::Matrix mb = {{{0.8951, 0.2664, -0.1614},
...
std::array<double, 2> xyzToXy(const DCPProfile::Triple &xyz)
{
    ...
    return total > 0.0 ? std::array<double, 2>{xyz[0] / total, xyz[1] / total}
                       : std::array<double, 2>{0.3457, 0.3585};
}
...
double xyCoordToTemperature(const std::array<double, 2> &white_xy)
{
    ...
    for (uint32_t index = 1; index <= 30; ++index) {
        ...
        if (dt <= 0.0 || index == 30) {
            ...
            res = 1.0e6 / (temp_table[index - 1].r * f +
...
std::map<std::string, std::string> getAliases(const Glib::ustring &profile_dir)
{
    const std::unique_ptr<std::FILE, std::function<void(std::FILE *)>> file(
        g_fopen(Glib::build_filename(profile_dir, "camera_model_aliases.json")
                    .c_str(),
                "rb"),
        [](std::FILE *file) { std::fclose(file); });
    ...
    if (!root || !root->child) {
        if (settings->verbose) {
            std::cout << "Could not parse 'camera_model_aliases.json' file."
                      << std::endl;
        }
        return {};
    }
            ...
            for (std::size_t index = 0; index < array_size; ++index) {
                ...
                if (cJSON_IsString(alias)) {
                    ...
                    if (settings->verbose > 1) {
                        std::cout << "dcp - alias: " << alias->valuestring
                                  << " -> " << camera->string << std::endl;
                    }
                }
            }
        }
...
public:
    ...
    bool parse()
    {
        ...
        if (numtags <= 0 ||
            numtags >
                1000) { // KodakIfd has lots of tags, thus 1000 as the limit
            return false;
        }
    ...
    bool find(int id) const { return tags_.find(id) != tags_.end(); }
    ...
    int toShort(int id, int ofs = 0) { return toInt(id, ofs, SHORT); }
    ...
    double toDouble(int id, int ofs = 0)
    {
        ...
        case FLOAT:
            ...
            return conv
                .f; // IEEE FLOATs are already C format, they just need a recast
...
private:
    ...
    bool parse_tag(Tag &t, FILE *f, int base, ByteOrder order)
    {
        ...
        // note the large count is to be able to pass LeafData ASCII tag which
        // can be up to almost 10 megabytes, (only a small part of it will
        // actually be parsed though)
...
DCPProfile::ApplyState::ApplyState(): data(new Data{}) {}

DCPProfile::ApplyState::~ApplyState() {}

DCPProfile::DCPProfile(const Glib::ustring &filename)
    : has_color_matrix_1(false), has_color_matrix_2(false),
      has_forward_matrix_1(false), has_forward_matrix_2(false),
      has_tone_curve(false), has_baseline_exposure_offset(false),
      will_interpolate(false), valid(false), baseline_exposure_offset(0.0)
{
    delta_info.hue_step = delta_info.val_step = look_info.hue_step =
        look_info.val_step = 0;
    ...
    enum TagKey {
        ...
        PROFILE_LOOK_TABLE_DIMS =
            50981, // ProfileLookup is the low quality variant
    ...
    light_source_1 = md.find(CALIBRATION_ILLUMINANT_1)
                         ? md.toShort(CALIBRATION_ILLUMINANT_1)
                         : -1;
    light_source_2 = md.find(CALIBRATION_ILLUMINANT_2)
    ...
    const bool has_second_hue_sat =
        md.find(PROFILE_HUE_SAT_MAP_DATA_2); // Some profiles have two matrices,
                                             // but just one huesat
    ...
    if (has_forward_matrix_1) {
        for (int row = 0; row < 3; ++row) {
            for (int col = 0; col < 3; ++col) {
                forward_matrix_1[row][col] =
                    md.toDouble(FORWARD_MATRIX_1, (col + row * 3) * 8);
            }
        }
    }
    ...
    if (has_forward_matrix_2) {
        for (int row = 0; row < 3; ++row) {
            for (int col = 0; col < 3; ++col) {
                forward_matrix_2[row][col] =
                    md.toDouble(FORWARD_MATRIX_2, (col + row * 3) * 8);
            }
        }
    }
    ...
    if (!md.find(COLOR_MATRIX_1)) {
        if (settings->verbose) {
            std::cerr << "DCP '" << filename
                      << "' is missing 'ColorMatrix1'. Skipped." << std::endl;
        }
        fclose(file);
        return;
    }
    ...
    for (int row = 0; row < 3; ++row) {
        for (int col = 0; col < 3; ++col) {
            color_matrix_1[row][col] =
                md.toDouble(COLOR_MATRIX_1, (col + row * 3) * 8);
        }
    }
    ...
    if (md.find(PROFILE_LOOK_TABLE_DIMS)) {
        ...
        look_info.srgb_gamma = md.find(PROFILE_LOOK_TABLE_ENCODING) &&
                               md.toInt(PROFILE_LOOK_TABLE_ENCODING);
        ...
        for (unsigned int i = 0; i < look_info.array_count; i++) {
            look_table[i].hue_shift =
                md.toDouble(PROFILE_LOOK_TABLE_DATA, (i * 3) * tiff_float_size);
            look_table[i].sat_scale = md.toDouble(
                PROFILE_LOOK_TABLE_DATA, (i * 3 + 1) * tiff_float_size);
            look_table[i].val_scale = md.toDouble(
                PROFILE_LOOK_TABLE_DATA, (i * 3 + 2) * tiff_float_size);
        }
    ...
    if (md.find(PROFILE_HUE_SAT_MAP_DIMS)) {
        ...
        delta_info.srgb_gamma = md.find(PROFILE_HUE_SAT_MAP_ENCODING) &&
                                md.toInt(PROFILE_HUE_SAT_MAP_ENCODING);
        ...
        for (unsigned int i = 0; i < delta_info.array_count; ++i) {
            deltas_1[i].hue_shift = md.toDouble(PROFILE_HUE_SAT_MAP_DATA_1,
                                                (i * 3) * tiff_float_size);
            deltas_1[i].sat_scale = md.toDouble(PROFILE_HUE_SAT_MAP_DATA_1,
                                                (i * 3 + 1) * tiff_float_size);
            deltas_1[i].val_scale = md.toDouble(PROFILE_HUE_SAT_MAP_DATA_1,
                                                (i * 3 + 2) * tiff_float_size);
        }
        ...
        delta_info.pc.val_step =
            delta_info.hue_divisions * delta_info.pc.hue_step;
    }
    ...
    if (light_source_2 != -1) {
        ...
        for (int row = 0; row < 3; ++row) {
            for (int col = 0; col < 3; ++col) {
                color_matrix_2[row][col] =
                    cm2 ? md.toDouble(COLOR_MATRIX_2, (col + row * 3) * 8)
                        : color_matrix_1[row][col];
            }
        }
        ...
        if (has_second_hue_sat) {
            ...
            for (unsigned int i = 0; i < delta_info.array_count; ++i) {
                deltas_2[i].hue_shift = md.toDouble(PROFILE_HUE_SAT_MAP_DATA_2,
                                                    (i * 3) * tiff_float_size);
                deltas_2[i].sat_scale = md.toDouble(
                    PROFILE_HUE_SAT_MAP_DATA_2, (i * 3 + 1) * tiff_float_size);
                deltas_2[i].val_scale = md.toDouble(
                    PROFILE_HUE_SAT_MAP_DATA_2, (i * 3 + 2) * tiff_float_size);
            }
        }
    }
    ...
    if (md.find(PROFILE_TONE_CURVE)) {
        ...
        for (unsigned int i = 0, n = md.getCount(PROFILE_TONE_CURVE); i < n;
             i += 2) {
            const double x =
                md.toDouble(PROFILE_TONE_CURVE, (i + 0) * tiff_float_size);
            const double y =
                md.toDouble(PROFILE_TONE_CURVE, (i + 1) * tiff_float_size);
        ...
        if (md.find(PROFILE_TONE_COPYRIGHT) &&
            md.toString(PROFILE_TONE_COPYRIGHT).find("Adobe Systems") !=
                std::string::npos) {
...
DCPProfile::operator bool() const { return has_color_matrix_1; }

bool DCPProfile::getHasToneCurve() const { return has_tone_curve; }

bool DCPProfile::getHasLookTable() const { return !look_table.empty(); }

bool DCPProfile::getHasHueSatMap() const { return !deltas_1.empty(); }
...
{
    return {light_source_1, light_source_2, temperature_1, temperature_2,
            will_interpolate};
}

void DCPProfile::apply(Imagefloat *img, int preferred_illuminant,
                       ...
                       bool apply_look_table, bool multithread) const
{
    ...
    const TMatrix work_matrix =
        ICCStore::getInstance()->workingSpaceInverseMatrix(working_space);

    const std::vector<HsbModify> delta_base =
        makeHueSatMap(white_balance, preferred_illuminant);
    ...
    const Matrix xyz_cam = makeXyzCam(
        white_balance, pre_mul, cam_wb_matrix, preferred_illuminant,
        apply_hue_sat_map || apply_look_table); // Camera RGB to XYZ D50 matrix

    if (!apply_hue_sat_map) {
        // The fast path: No LUT --> Calculate matrix for direct conversion raw
        // -> working space
        float mat[3][3] = {};
...
#pragma omp parallel for if (multithread)
        ...
        for (int y = 0; y < img->getHeight(); ++y) {
            for (int x = 0; x < img->getWidth(); x++) {
                const float &newr = mat[0][0] * img->r(y, x) +
                                    mat[0][1] * img->g(y, x) +
                                    mat[0][2] * img->b(y, x);
                const float &newg = mat[1][0] * img->r(y, x) +
                                    mat[1][1] * img->g(y, x) +
                                    mat[1][2] * img->b(y, x);
                const float &newb = mat[2][0] * img->r(y, x) +
                                    mat[2][1] * img->g(y, x) +
                                    mat[2][2] * img->b(y, x);
...
#pragma omp parallel for schedule(dynamic, 16) if (multithread)
        ...
        for (int y = 0; y < img->getHeight(); ++y) {
            for (int x = 0; x < img->getWidth(); x++) {
                float newr = pro_photo[0][0] * img->r(y, x) +
                             pro_photo[0][1] * img->g(y, x) +
                             pro_photo[0][2] * img->b(y, x);
                float newg = pro_photo[1][0] * img->r(y, x) +
                             pro_photo[1][1] * img->g(y, x) +
                             pro_photo[1][2] * img->b(y, x);
                float newb = pro_photo[2][0] * img->r(y, x) +
                             pro_photo[2][1] * img->g(y, x) +
                             pro_photo[2][2] * img->b(y, x);

                // If point is in negative area, just the matrix, but not the
                // LUT. This is checked inside Color::rgb2hsvdcp
                float h;
                float s;
                float v;
                ...
                img->r(y, x) =
                    work[0][0] * newr + work[0][1] * newg + work[0][2] * newb;
                img->g(y, x) =
                    work[1][0] * newr + work[1][1] * newg + work[1][2] * newb;
                img->b(y, x) =
                    work[2][0] * newr + work[2][1] * newg + work[2][2] * newb;
            }
        }
    }
}

void DCPProfile::setStep2ApplyState(const Glib::ustring &working_space,
                                    bool use_tone_curve, bool apply_look_table,
                                    bool apply_baseline_exposure,
                                    ApplyState &as_out)
{
    ...
    if (working_space == "ProPhoto") {
        ...
        for (int i = 0; i < 3; i++)
            for (int j = 0; j < 3; j++)
                for (int k = 0; k < 3; k++) {
                    as_out.data->pro_photo[i][j] +=
                        prophoto_xyz[i][k] * mWork[k][j];
                }
...
void DCPProfile::step2ApplyTile(float *rc, float *gc, float *bc, int width,
                                int height, int tile_width,
                                const ApplyState &as_in) const
{
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                if (as_in.data->already_pro_photo) {
                    ...
                    newr = as_in.data->pro_photo[0][0] * r +
                           as_in.data->pro_photo[0][1] * g +
                           as_in.data->pro_photo[0][2] * b;
                    newg = as_in.data->pro_photo[1][0] * r +
                           as_in.data->pro_photo[1][1] * g +
                           as_in.data->pro_photo[1][2] * b;
                    newb = as_in.data->pro_photo[2][0] * r +
                           as_in.data->pro_photo[2][1] * g +
                           as_in.data->pro_photo[2][2] * b;
                }

                // with looktable and tonecurve we need to clip
                if (as_in.data->apply_look_table ||
                    as_in.data->use_tone_curve) {
                    newr = max(newr, 0.f);
                    newg = max(newg, 0.f);
                    newb = max(newb, 0.f);
                }
                ...
                if (as_in.data->already_pro_photo) {
                    ...
                    rc[y * tile_width + x] = as_in.data->work[0][0] * newr +
                                             as_in.data->work[0][1] * newg +
                                             as_in.data->work[0][2] * newb;
                    gc[y * tile_width + x] = as_in.data->work[1][0] * newr +
                                             as_in.data->work[1][1] * newg +
                                             as_in.data->work[1][2] * newb;
                    bc[y * tile_width + x] = as_in.data->work[2][0] * newr +
                                             as_in.data->work[2][1] * newg +
                                             as_in.data->work[2][2] * newb;
                }
            }
        }
    }
...
DCPProfile::Matrix
DCPProfile::findXyztoCamera(const std::array<double, 2> &white_xy,
                            int preferred_illuminant, double wbtemp) const
{
    ...
    if (has_col_1 && has_col_2) {
        /*
          Note: We're using DNG SDK reference code for XY to temperature
          translation to get the exact same mix as the reference code does.
        */
...
std::array<double, 2> DCPProfile::neutralToXy(const Triple &neutral,
                                              ...
                                              double wbtemp) const
{
    ...
    for (unsigned int pass = 0; pass < MAX_PASSES; ++pass) {
        const Matrix &xyz_to_camera =
            findXyztoCamera(last_xy, preferred_illuminant, wbtemp);
        ...
        if (std::fabs(next_xy[0] - last_xy[0]) +
                std::fabs(next_xy[1] - last_xy[1]) <
            0.0000001) {
            return next_xy;
        }
...
DCPProfile::Matrix DCPProfile::makeXyzCam(const ColorTemp &white_balance,
                                          ...
                                          bool use_fwd_matrix) const
{
    ...
    // Note that we do not support monochrome or colorplanes > 3 (no
    // reductionMatrix support), we do not support cameracalibration either.

    Triple neutral; // Same as the DNG "AsShotNeutral" tag if white balance is
                    // Camera's own
    {
        /* A bit messy matrixing and conversions to get the neutral[] array from
           RT's own white balance which is stored in sRGB space, while the DCP
           code needs multipliers in CameraRGB space */
        double r, g, b;
        ...
        double camwb_red =
            cam_rgb[0][0] * r + cam_rgb[0][1] * g + cam_rgb[0][2] * b;
        double camwb_green =
            cam_rgb[1][0] * r + cam_rgb[1][1] * g + cam_rgb[1][2] * b;
        double camwb_blue =
            cam_rgb[2][0] * r + cam_rgb[2][1] * g + cam_rgb[2][2] * b;
    ...
    /* Calculate what the RGB multipliers corresponds to as a white XY
       coordinate, based on the DCP ColorMatrix or ColorMatrices if
       dual-illuminant. This is the DNG reference code way to do it, which is a
       bit different from RT's own white balance model at the time of writing.
       When RT's white balance can make use of the DCP color matrices we could
       use that instead. */
    const std::array<double, 2> white_xy =
        neutralToXy(neutral, preferred_illuminant, wbtemp);
    ...
    if ((has_col_1 && has_col_2) || (has_fwd_1 && has_fwd_2)) {
        /* DNG ref way to convert XY to temperature, which affect matrix mixing.
           A different model here typically does not affect the result too much,
           ie it's probably not strictly necessary to use the DNG reference code
           here, but we do it for now. */
        // const double wbtemp = xyCoordToTemperature(white_xy);
        // const double wbtemp = white_balance.getTemp();
    ...
    if (has_col_1 && has_col_2) {
        ...
        if (mix >= 1.0) {
            ...
            color_matrix =
                mix3x3(color_matrix_1, mix, color_matrix_2, 1.0 - mix);
        }
    } else if (has_col_1) {
        color_matrix = color_matrix_1;
    } else {
    ...
    /*
      The exact position of the white XY coordinate affects the result very
      much, thus it's important that the result is very similar or the same as
      DNG reference code. Especially important is it that the raw-embedded
      "AsShot" multipliers is translated to the same white XY coordinate as the
      DNG reference code, or else third party DCPs will show incorrect color.
    */
    ...
    if (has_fwd_1 || has_fwd_2) {
        ...
        if (has_fwd_1 && has_fwd_2) {
            ...
            if (mix >= 1.0) {
                ...
                fwd =
                    mix3x3(forward_matrix_1, mix, forward_matrix_2, 1.0 - mix);
            }
        } else if (has_fwd_1) {
            fwd = forward_matrix_1;
        } else {
        ...
        const Matrix white_diag = {{{camera_white[0], 0, 0},
                                    {0, camera_white[1], 0},
                                    {0, 0, camera_white[2]}}};
        ...
        cam_xyz =
            multiply3x3(color_matrix, mapWhiteMatrix(white_d50, white_xyz));
    }

    // Convert cam_xyz (XYZ D50 to CameraRGB, "PCS to Camera" in DNG
    // terminology) to mXYZCAM

    // This block can probably be simplified, seems unnecessary to pass through
    // the sRGB matrix (probably dcraw legacy), it does no harm though as we
    // don't clip anything.
    int i, j, k;
...
std::vector<DCPProfile::HsbModify>
DCPProfile::makeHueSatMap(const ColorTemp &white_balance,
                          int preferred_illuminant) const
{
    ...
    if (temperature_1 <= 0.0 || temperature_2 <= 0.0 ||
        temperature_1 == temperature_2) {
        return deltas_1;
    }

    const bool reverse = temperature_1 > temperature_2;
    const double t1 = reverse ? temperature_2 : temperature_1;
    const double t2 = reverse ? temperature_1 : temperature_2;
    ...
    for (unsigned int i = 0; i < delta_info.array_count; ++i) {
        res[i].hue_shift = hue_interpolate(w1, deltas_1[i].hue_shift, w2,
                                           deltas_2[i].hue_shift);
        res[i].sat_scale =
            w1 * deltas_1[i].sat_scale + w2 * deltas_2[i].sat_scale;
        res[i].val_scale =
            w1 * deltas_1[i].val_scale + w2 * deltas_2[i].val_scale;
    }

    return res;
}

void DCPProfile::hsdApply(const HsdTableInfo &table_info,
                          const std::vector<HsbModify> &table_base, float &h,
                          float &s, float &v) const
{
    ...
    if (table_info.val_divisions < 2) {
        ...
        const int s_index0 =
            std::max(std::min<int>(s_scaled, table_info.pc.max_sat_index0), 0);
        ...
        std::vector<HsbModify>::size_type e00_index =
            h_index0 * table_info.pc.hue_step + s_index0;
        std::vector<HsbModify>::size_type e01_index =
            e00_index + (h_index1 - h_index0) * table_info.pc.hue_step;

        const float hue_shift0 =
            hue_interpolate(h_fract0, table_base[e00_index].hue_shift, h_fract1,
                            table_base[e01_index].hue_shift);
        const float sat_scale0 = h_fract0 * table_base[e00_index].sat_scale +
                                 h_fract1 * table_base[e01_index].sat_scale;
        const float val_scale0 = h_fract0 * table_base[e00_index].val_scale +
                                 h_fract1 * table_base[e01_index].val_scale;
        ...
        const float hueShift1 =
            hue_interpolate(h_fract0, table_base[e00_index].hue_shift, h_fract1,
                            table_base[e01_index].hue_shift);
        const float satScale1 = h_fract0 * table_base[e00_index].sat_scale +
                                h_fract1 * table_base[e01_index].sat_scale;
        const float valScale1 = h_fract0 * table_base[e00_index].val_scale +
                                h_fract1 * table_base[e01_index].val_scale;
        ...
        const int s_index0 =
            std::max(std::min<int>(s_scaled, table_info.pc.max_sat_index0), 0);
        const int v_index0 =
            std::max(std::min<int>(v_scaled, table_info.pc.max_val_index0), 0);
        ...
        std::vector<HsbModify>::size_type e00_index =
            v_index0 * table_info.pc.val_step +
            h_index0 * table_info.pc.hue_step + s_index0;
        std::vector<HsbModify>::size_type e01_index =
            e00_index + (h_index1 - h_index0) * table_info.pc.hue_step;
        std::vector<HsbModify>::size_type e10_index =
            e00_index + table_info.pc.val_step;
        std::vector<HsbModify>::size_type e11_index =
            e01_index + table_info.pc.val_step;

        const float hueShift0 = hue_interpolate(
            v_fract0,
            hue_interpolate(h_fract0, table_base[e00_index].hue_shift, h_fract1,
                            table_base[e01_index].hue_shift),
            v_fract1,
            hue_interpolate(h_fract0, table_base[e10_index].hue_shift, h_fract1,
                            table_base[e11_index].hue_shift));
        const float satScale0 =
            v_fract0 * (h_fract0 * table_base[e00_index].sat_scale +
                        h_fract1 * table_base[e01_index].sat_scale) +
            v_fract1 * (h_fract0 * table_base[e10_index].sat_scale +
                        h_fract1 * table_base[e11_index].sat_scale);
        const float valScale0 =
            v_fract0 * (h_fract0 * table_base[e00_index].val_scale +
                        h_fract1 * table_base[e01_index].val_scale) +
            v_fract1 * (h_fract0 * table_base[e10_index].val_scale +
                        h_fract1 * table_base[e11_index].val_scale);
        ...
        const float hueShift1 = hue_interpolate(
            v_fract0,
            hue_interpolate(h_fract0, table_base[e00_index].hue_shift, h_fract1,
                            table_base[e01_index].hue_shift),
            v_fract1,
...
bool DCPProfile::isValid() { return valid; }
        ...
        for (const Glib::ustring &sname : *dir) {
            ...
            if (!Glib::file_test(fname, Glib::FILE_TEST_IS_DIR)) {
                ...
                if (lastdot != Glib::ustring::npos &&
                    ...
                    const Glib::ustring cam_short_name =
                        sname.substr(0, lastdot).uppercase();
                    file_std_profiles[cam_short_name] =
                        fname; // They will be loaded and cached on demand
                }
            } else {
                // Directory
...
DCPProfile *
DCPStore::getCameraProfile(const Glib::ustring &requested_cam_short_name) const
{
    const Glib::ustring name = requested_cam_short_name.uppercase();

    // Warning: do NOT use map.find(), since it does not seem to work reliably
    // here
    ...
    for (const auto &dir : profileDir) {
        if (!dir.empty()) {
            const Glib::ustring fname = Glib::build_filename(
                dir, requested_cam_short_name + Glib::ustring(".dcp"));
