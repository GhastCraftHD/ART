...
void WavOpacityCurveWL::Reset() { lutOpacityCurveWL.reset(); }

void WavOpacityCurveWL::Set(const Curve &pCurve)
{
    if (pCurve.isIdentity()) {
        lutOpacityCurveWL.reset(); // raise this value if the quality suffers
                                   // from this number of samples
        return;
    }

    lutOpacityCurveWL(501); // raise this value if the quality suffers from this
                            // number of samples
...
void eval_avg(float *RESTRICT DataList, int datalen, float &averagePlus,
              float &averageNeg, float &max, float &min, bool multiThread)
{
    // find absolute mean
    int countP = 0, countN = 0;
    double averaP = 0.0,
           averaN = 0.0; // use double precision for large summations

    float thres =
        5.f; // different fom zero to take into account only data large enough
    max = 0.f;
    min = 0.f;
...
void eval_sigma(float *RESTRICT DataList, int datalen, float averagePlus,
                float averageNeg, float &sigmaPlus, float &sigmaNeg,
                bool multiThread)
{
    int countP = 0, countN = 0;
    double variP = 0.0,
           variN = 0.0; // use double precision for large summations
    float thres =
        5.f; // different fom zero to take into account only data large enough

#ifdef _OPENMP
#pragma omp parallel for reduction(+ : variP, variN, countP,                   \
                                       countN) if (multiThread)
#endif
    for (int i = 0; i < datalen; i++) {
        if (DataList[i] >= thres) {
...
void eval_level(float **wl, int level, int W_L, int H_L, float *mean,
                float *meanN, float *sigma, float *sigmaN, float *MaxP,
                float *MaxN, bool multiThread)
{
    float avLP[4], avLN[4];
    float maxL[4], minL[4];
    float sigP[4], sigN[4];
    ...
    for (int dir = 1; dir < 4; dir++) {
        eval_avg(wl[dir], W_L * H_L, avLP[dir], avLN[dir], maxL[dir], minL[dir],
                 multiThread);
        eval_sigma(wl[dir], W_L * H_L, avLP[dir], avLN[dir], sigP[dir],
                   sigN[dir], multiThread);
    }
...
void evaluate_params(wavelet_decomposition &wd, float *mean, float *meanN,
                     float *sigma, float *sigmaN, float *MaxP, float *MaxN,
                     bool multiThread)
{
    ...
    for (int lvl = 0; lvl < maxlvl; lvl++) {
        ...
        eval_level(wl, lvl, Wlvl_L, Hlvl_L, mean, meanN, sigma, sigmaN, MaxP,
                   MaxN, multiThread);
    }
}

void local_contrast_wavelets(array2D<float> &Y,
                             const LocalContrastParams::Region &params,
                             double scale, bool multiThread)
{
    const int W = Y.width();
    ...
    int skip = scale;
    wavelet_decomposition wd(static_cast<float *>(Y), W, H, wavelet_level, 1,
                             skip);
...
#pragma omp parallel for if (multiThread)
    ...
    for (int dir = 1; dir < 4; dir++) {
        for (int level = 0; level < maxlvl; ++level) {
            ...
            if (MaxP[level] > 0.f && mean[level] != 0.f &&
                sigma[level] != 0.f) {
                float insigma = 0.666f;          // SD
                float logmax = log(MaxP[level]); // log Max
                float rapX = (mean[level] + sigma[level]) /
                             MaxP[level]; // rapport between sD / max
...
bool ImProcFunctions::localContrast(Imagefloat *rgb)
{
    ...
    if (params->localContrast.enabled) {
        ...
        if (!generateMasks(
                rgb, "localcontrast", linked_mask_mgr_,
                params->localContrast.masks, offset_x, offset_y, full_width,
